...
## ওয়াকফ চিহ্ন — দুটো ফাঁদ, দুটোই engine-এ সারানো

আরবি পাঠ যাচাইকৃত ডেটাসেট থেকে আসে, কিন্তু **রেন্ডারিংয়ে** দুটো সমস্যা ছিল
(দুটোরই ফিক্স engine-এ, এই রিপোতে নয় — `engine/build.py` ও `templates/base.css`):

1. **`AlQuranIndoPak`-এর ওয়াকফ glyph ভাঙা।** ফন্টে cmap এন্ট্রি আছে (তাই
   `font_report.json` বলে "missing: 0", আর সেটা ভুলও নয়) কিন্তু `U+06D8`
   (ওয়াকফ লাযিম — অবশ্যই থামতে হবে) **কালো বাক্স** আঁকত, `U+06D7` শুধু ق,
   `U+06D6` কাটা صل, `U+06DC` কিছুই না। **cmap-এ এন্ট্রি থাকাটাই ফাঁদ** —
   ব্রাউজার তখন fallback ফন্টে যায় না। `FONT_UNICODE_RANGE` দিয়ে IndoPak-এর
   রেঞ্জ সংকুচিত করে ওয়াকফ ব্লকটা `Noto Naskh Arabic`-কে দেওয়া হয়েছে।
2. **চিহ্নগুলো পড়ার মতো বড় ছিল না।** Naskh ওগুলো Naskh-এর মাপে আঁকে, যা
   ইন্দো-পাক অক্ষরের পাশে খুব ছোট। `|waqf` Jinja filter প্রতিটা চিহ্নকে
   `<span class="waqf">`-এ মোড়ায়, আর CSS ২০০% বড় করে।

**শুধু font-size বাড়ালে হয় না** — ওয়াকফ চিহ্ন zero-advance mark, বড় করলে
baseline থেকে দূরত্বও বাড়ে আর চিহ্ন লাইনের অনেক উপরে উঠে শব্দ থেকে আলাদা হয়ে
যায়। তাই সাথে `line-height: 0` (লাইন-বক্স যেন না বাড়ে) আর
`top: 0.55em` (নামিয়ে আনতে) লাগে।

span মোড়ানো হয় **filter-এ, `quran.py`-তে নয়** — একই `ar` স্ট্রিং DOCX writer-ও
খায়, ওখানে করলে Word ফাইলে literal `<span>` ট্যাগ ঢুকে যেত।
